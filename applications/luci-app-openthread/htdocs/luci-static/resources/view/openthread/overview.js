...
// deserves the same caveats before it fires. Blocking with the filter off
// also turns it on, which is a router-wide change and has to be on screen
// before it is claimed as acknowledged.
var blockWarning = function(activatesDenylist) {
	var w = [];
	if (activatesDenylist)
		w.push(_('The MAC filter is currently off on this border router. Blocking this device also switches it to denylist mode, a router-wide setting that from then on decides which devices are refused.'),
			E('br'), E('br'));
	w.push(
...
function handleBlock(bss, neigh) {
	...
	// The row is rendered either with the filter off or with the denylist
	// already on; only the first case is a mode change, and only there does
	// the modal spell one out.
	var activates = (neigh.macfilter == 'disable');
	filterModal(_('Block device'),
		[ _('Stop accepting traffic from'), ' ', E('strong', {}, [ name ]), ' ',
			_('on this border router.') ],
		blockWarning(activates),
		...
		function() {
			...
			// Already and leaves the mode alone next to leftovers. The
			// acknowledgement the backend's mode-activation guard asks for
			// is only ours to give when the modal named the switch, so it
			// is true exactly then: if the mode drifted under a
			// two-second-old row, unconfirmed_mode is the right answer.
			return callMacfilterAdd(bss.ExtAddress, 'denylist', activates);
		});
}
...
function handleAllowlistBlock(bss) {
	...
	filterModal(_('Block device'),
		...
		_('Block'), 'cbi-button cbi-button-negative',
		function() {
			// The row was rendered under the allowlist, so this changes one
			// entry and no mode. Nothing to confirm.
...
function handleAllow(bss) {
	...
	filterModal(_('Allow device'),
		...
		function() {
			// Same here: the allowlist is already on, so no confirmation is
			// due. Sending one would let a stale row turn the allowlist on
			// with this device as its only entry, which blocks the rest of
			// the mesh -- the very thing the guard exists for.
